...
const Strava: FunctionComponent = () => {
  ...
  return (
    <div className={styles.stravaWrapper}>
      {!data ? (
        <Cycling />
      ) : (
        <div className="w-full grid grid-cols-1 md:grid-cols-2 md:gap-x-16 lg:gap-x-32 gap-y-20 md:gap-y-32">
    ...
    <div className={styles.stravaCard}>
      ...
      <div className="strava-card-body">
        <a href={`https://www.strava.com/activities/${sportDetail.id}`} target="_blank" rel="noopener noreferrer">
          <div
            className={styles.latestRun + ' ' + styles.hasPhoto}
            style={{
              backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.25), transparent 25%, transparent 60%, rgba(0, 0, 0, 0.5)), url('${sportDetail.photoUrl}')`,
            }}
          >
            <div className={styles.activityName}>{sportDetail.name}</div>
            {!sportDetail.photoUrl ? <div className="flex justify-center text-slate-400">No photo 🙁</div> : undefined}
